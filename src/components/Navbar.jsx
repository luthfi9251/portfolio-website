...
export default function Navbar() {
  ...
  return (
    <>
      <motion.div
        className={`text-white md:hidden top-0 left-2 h-12 sticky flex items-center z-50 w-screen bg-c-dark-background`}
      >
        <span
          className={`${
            isOpen ? "hidden" : "block"
        ...
        <span
          className={` ${
            isOpen ? "block" : "hidden"
          } fixed top-3 right-4 text-xl font-bold text-white md:hidden`}
          onClick={openNavbar}
        >
      ...
      <motion.nav
        className={`flex text-slate-200 justify-center md:justify-between px-3 md:h-14 items-center flex-col md:flex-row h-screen w-screen md:w-auto gap-3 md:static overflow-y-hidden fixed ${
      ...
      >
        ...
        <div className="flex items-center w-20 md:w-48 justify-center md:justify-end gap-x-1 md:h-3/5">
          <a href="" className="h-full">
            <Image src={githubIcon} className="w-full h-full object-cover" />
          </a>
          <a href="" className="h-full">
            <Image src={linkedinIcon} className="w-full h-full object-cover" />
          </a>
        </div>
      </motion.nav>
    </>
