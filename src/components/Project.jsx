...
export default function Project() {
  ...
  useEffect(() => {
    switch (active) {
      case 0:
        setData(
          dataInitial.filter((item) => item.kategori === "web development")
        );
        break;
      case 1:
        setData(
          dataInitial.filter((item) => item.kategori === "android development")
        );
        break;
      case 2:
        setData(
          dataInitial.filter((item) => item.kategori === "python project")
        );
        break;
      case 3:
        setData(dataInitial.filter((item) => item.kategori === "iot project"));
        break;
      default:
        setData(dataInitial);
        break;
  ...
  return (
    <>
      {isModalOpen && (
        <ModalProject handleClose={handleCloseModal} item={activeDataModal} />
      )}
      <div className=" w-full py-10 px-3 md:px-6 my-8 md:my-0">
        <div className="flex flex-col items-center gap-4">
          ...
          <div className="grid grid-cols-4 gap-x-1">
            <button
              className={`w-full ${
                active === 0 ? "bg-c-dark-green" : "bg-c-dark-grey text-white"
            ...
            <button
              className={`w-full ${
                active === 1 ? "bg-c-dark-green" : "bg-c-dark-grey text-white"
            ...
            <button
              className={`w-full ${
                active === 2 ? "bg-c-dark-green" : "bg-c-dark-grey text-white"
              } text-center hover:bg-c-dark-green transition-all py-3 px-1 md:px-4 font-medium text-xs md:text-base`}
            ...
            <button
              className={`w-full ${
                active === 3 ? "bg-c-dark-green" : "bg-c-dark-grey text-white"
        ...
        <div className="w-full flex md:py-8 justify-center items-stretch flex-wrap gap-4">
          <div className=" py-2 grid grid-cols-1 md:grid-cols-3 w-full gap-3">
            {data.map((item, index) => {
              return (
                <div className="group w-full h-56 relative rounded-md overflow-hidden">
                  <Image
                    src={item.img}
